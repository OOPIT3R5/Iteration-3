...
public class InventoryController extends Controller {
	...
	private class UseItem implements MouseListener {
		...
		private final int EquipmentStartingX = InventoryView.INV_WIDTH
				+ InventoryView.EQUIP_WIDTH / 3 - InventoryView.INV_GRIDWIDTH
				/ 6;// = 1163;
		private final int EquipmentStartingY = InventoryView.INV_GRIDY;// = 131;

		private final int EquipmentMidXLo = InventoryView.INV_WIDTH
				+ InventoryView.EQUIP_WIDTH / 3; // 1251
		...
		private void checkSkills(int X, int Y) {
			...
			if ((X < (InventoryView.INV_GRIDX + (InventoryView.BUTTONWIDTH * 6)))
					|| (Y < InventoryView.STATS_HEIGHT / 5
							+ InventoryView.STATS_Y + 31)) {
			...
			if (((X * 4) + Y) < skillamount && e.getskillPoints() > 0) {
				...
				System.out.println("Skill " + skills.get((X * 4) + Y).getName()
						+ " leveled up!");
				e.useSkillPoint();
			}
